...
void dlms_actions_func(void)
{
	unsigned int i, j, k;
		...
		{
				for(i=0;i<16;i++)
							 write_eeprom(CAL_NAME_PASSIVE_LOC+i,Calendar_Name_Passive[i]);
				set_cal_name_data=0;
		}
		...
		{
			i=time_string[0];
			i=(i*256)+time_string[1];
			
			s_time.tm_subsec = 0; // A break can be set here.
			s_time.tm_sec = time_string[7]; 
			s_time.tm_min = time_string[6]; 
			s_time.tm_hour = time_string[5];
			s_time.tm_mday = time_string[3]; 
			
			if(time_string[2]>0)
				s_time.tm_mon = time_string[2]-1; 
			else
				s_time.tm_mon = 0; 
				
			s_time.tm_year = i%100; 
			s_time.tm_wday = SUN;
			s_time.tm_subsec = 0;
			rtc_write(&s_time);
		...
		{
			get_time_data(real_date, real_time);
			memcpy(time_string,tmp_time_string,12);
		}  
