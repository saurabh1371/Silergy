...
#define KWh_Icon 	lcd_put_icon(_k);\
									lcd_put_icon(_v);\
									lcd_put_icon(T1);\
									lcd_put_icon(T2);\
									lcd_put_icon(_h);

#define KW_Icon  	lcd_put_icon(_k);\
									lcd_put_icon(_v);\
									lcd_put_icon(T1);\
									lcd_put_icon(T2);

#define V_Icon 		lcd_put_icon(_v);

#define A_Icon 		lcd_put_icon(T2);\
									lcd_put_icon(T3);
...
const  char test_str[][15] =
{
    ...
    "PF","LG","LD","LAB","P","N","EE","V","SP","NP","SILERG4","COMM","CLr2","CLr3","CLr1","UnLoc","CLrc","rESEt","SLno","CAL","HZ"
};
